...
def metod_euler32(f , a, b, yi, h):
    y=np.float32(yi) #valor anterior
    t=np.float32(a)  # tiempo inicial
    m = int((b-a)/h)  # número de pasos
    lista = [(t, y)]
    for i in range(m):
        y = y + f(t, y) * h
...
def metod_taylor_segundo_orden32(f, dfdy, a, b, yi, h, dfdt=None):
    """Versión con precisión de 32 bits del método de Taylor de orden 2."""
    if dfdt is None:
        dfdt = lambda t, y: 0
...
def metod_ruger32(f, a, b, yi, h, orden):
    m = int((b-a)/h)
    y = np.float32(yi)
    t = np.float32(a)
    if orden == 4:
